...
test('a failed thumbnail never leaves the fullscreen viewer Tab loop',async({page})=>{
  ...
  await page.evaluate(()=>document.addEventListener('keydown',event=>{if(event.key==='Tab')document.documentElement.dataset.testTabPrevented=String(event.defaultPrevented);}));
  await page.keyboard.press('Tab');await expect(page.locator('html')).toHaveAttribute('data-test-tab-prevented','true');await expect(page.locator('#lightbox .lightbox-close')).toBeFocused();
