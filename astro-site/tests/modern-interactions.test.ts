import { waitForPhoto } from '../src/scripts/photo-ready';
import { safeInlineJson } from '../src/lib/safe-json';
...
describe('Modern interaction data and semantic contracts',()=>{
 ...
 it('comparison displays each real plan with its own occupancy and prices',()=>{const $=read('rooms/index.html');const option=$('[data-compare-room="campsite_1"] .compare-plan');expect(option.eq(1).text()).toContain('三帳包區 · 12 人以下');expect(option.eq(1).find('dd').map((_,e)=>$(e).text()).get()).toEqual(['NT$2,400','NT$3,000','NT$3,600']);expect(option.eq(0).text()).toContain('16 人以下');expect(option.eq(0).find('dd').map((_,e)=>$(e).text()).get()).toEqual(['NT$3,200','NT$4,000','NT$4,800']);});
...
it('inline JSON cannot terminate its HTML data block',()=>{
 const value={alt:'</script><img src=x onerror=alert(1)>'};
...
it('slow image readiness is not mislabeled as an image error',async()=>{
 const image=new EventTarget() as EventTarget & {complete:boolean;naturalWidth:number;decode:()=>Promise<void>};
 image.complete=false;image.naturalWidth=0;image.decode=()=>new Promise(()=>{});
 await expect(waitForPhoto(image as unknown as HTMLImageElement,5)).resolves.toBe('timeout');
